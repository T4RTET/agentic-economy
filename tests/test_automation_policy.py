...
from app.repositories import create_agent
...
def _insert_attempt(db: sqlite3.Connection, agent_id: int, value_usd: float = 1, status: str = "prepared") -> None:
    db.execute(
        """
        INSERT INTO agent_automation_attempts
        (agent_id, action_type, to_address, value_wei, value_usd, chain_id, status)
        VALUES (?, 'native_transfer', ?, '10', ?, 5000, ?)
        """,
        (agent_id, RECIPIENT, value_usd, status),
    )
    db.commit()
...
def test_daily_limit_blocks() -> None:
    db, agent_id = _db_and_agent()
    try:
        _insert_attempt(db, agent_id, value_usd=9, status="executed")
...
def test_minimum_balance_blocks() -> None:
    db, agent_id = _db_and_agent()
    try:
        result = _evaluate(db, agent_id, _policy(min_native_balance_wei="5"), _action(value_wei="6"), current_native_balance_wei=10)
        assert result["allowed"] is False
