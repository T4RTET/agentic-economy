...
class AutomationPolicyUpdate(BaseModel):
    ...
    @classmethod
    def normalize_smart_account_address(cls, value: str | None) -> str | None:
...
class DelegationConfirmRequest(BaseModel):
    ...
    def validate_delegation_id(cls, value: str) -> str:
        delegation_id = value.strip()
        if not delegation_id:
            raise ValueError("delegation_id must not be empty")
        return delegation_id

    @field_validator("delegation_scope")
    @classmethod
    def validate_delegation_scope(cls, value: dict[str, Any]) -> dict[str, Any]:
        if not value:
            raise ValueError("delegation_scope must not be empty")
        return value
